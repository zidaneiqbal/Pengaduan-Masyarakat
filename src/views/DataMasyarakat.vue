        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <p style="font-family: sans-serif" class="card-title float-left"><b>Data Masyarakat :</b></p>
                  <b-button style="font-family: sans-serif" class="mdi mdi-plus card-outline float-right" size="m" v-on:click="addMasyarakat" v-b-modal.modalMasyarakat>
                    Tambah Masyarakat
                  </b-button>
                  <div class="table-responsive">
                    <b-table striped hover :items="masyarakat" :fields="fields">
                      <template v-slot:cell(Aksi)="data">
                        <b-button  class="mr-2" style="font-family: sans-serif" variant="secondary" size="sm" v-on:click="editMasyarakat(data.item.id)" v-b-modal.modalMasyarakat>
                            Edit
                        </b-button>
                        <b-button  class="mr-2" style="font-family: sans-serif" variant="danger" size="sm" v-on:click="hapusMasyarakat(data.item.id)">
                            Hapus
                        </b-button>
                      </template>
                    </b-table>
...
export default {
    data() {
      return {
        ...
        masyarakat: [],
        token : "",
        fields: ["nik", "username", "nama", "telp", "Aksi"]
      }
    },
    methods: {
        getMasyarakat : function(){
          let conf = { headers: { "Authorization" : 'Bearer ' + this.token } };
          this.$store.dispatch("getMasyarakat")
          .then(response => {
        ...
        editMasyarakat: function(id){
          this.action = "edit";
          this.$store.dispatch("getMasyarakatId", id)
          .then(response => {
        ...
        Save : function(){
          ...
          this.$bvModal.hide("modalMasyarakat");
          ...
          if (this.action == "insert") {
            this.$store.dispatch("addMasyarakat", form)
            .then(response => {
        ...
        hapusMasyarakat : function(id){
            let conf = { headers: { "Authorization" : 'Bearer ' + this.key } };
            if(confirm('Apakah anda yakin ingin menghapus data ini?')){
                this.message = "Mohon tunggu...";
                this.$store.dispatch("hapusMasyarakat", id)
                .then(response => {
                    this.getMasyarakat();
                    this.message = "";
                })
